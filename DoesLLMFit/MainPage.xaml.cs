...
public sealed partial class MainPage : Page
{
    ...
    private bool _isUpdatingOrgCombo;
    private bool _orgComboRebuildPending;
    private bool _gpuComboRebuildPending;
    ...
    public MainPage()
    {
        ...
        ViewModel.GpuItems.CollectionChanged += (_, _) =>
        {
            ...
            {
                _gpuComboRebuildPending = true;
                DispatcherQueue.TryEnqueue(() =>
                {
                    _gpuComboRebuildPending = false;
                    PopulateGpuComboBox();
                });
            }
        };
        ViewModel.OrganizationList.CollectionChanged += (_, _) =>
        {
            // Debounce: CollectionChanged fires for every Clear/Add;
            // dispatch a single rebuild after the batch completes.
            if (!_orgComboRebuildPending)
            {
                _orgComboRebuildPending = true;
                DispatcherQueue.TryEnqueue(() =>
                {
                    _orgComboRebuildPending = false;
                    PopulateOrgComboBox();
                });
            }
        };
        ...
        {
            if (e.PropertyName == nameof(ViewModel.SelectedGpu))
                SyncGpuComboBoxSelection();
            else if (e.PropertyName == nameof(ViewModel.SelectedOrganization))
                SyncOrgComboBoxSelection();
        };
    }
    ...
    private void PopulateGpuComboBox()
    {
        ...
        foreach (var gpuVm in ViewModel.GpuItems)
        {
            var (pathData, vbSize, brandColor) = GetGpuBrandInfo(gpuVm.Name);
    ...
    private void SyncGpuComboBoxSelection()
    {
        if (ViewModel.SelectedGpu is null)
        {
            GpuComboBox.SelectedItem = null;
            return;
        }
        foreach (ComboBoxItem item in GpuComboBox.Items)
        {
            if (item.Tag is GpuItemViewModel vm && vm.Name == ViewModel.SelectedGpu.Name)
            {
                GpuComboBox.SelectedItem = item;
                return;
            }
        }
    }
    ...
    private void GpuComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            ViewModel.SelectedGpu = gpuVm;
        }
    }

    // Keep: ContextLength ComboBox uses static ComboBoxItems with Tag
    private void ContextComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            ViewModel.ContextLength = ctx;
        }
    }

    // Keep: Category ComboBox uses static ComboBoxItems with Tag
    private void CategoryComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (CategoryComboBox.SelectedItem is ComboBoxItem item && item.Tag is string tag)
        {
    ...
    {
        _isUpdatingOrgCombo = true;
        try
        {
            OrgComboBox.Items.Clear();
            foreach (var orgName in ViewModel.OrganizationList)
            {
                ...
                sp.Children.Add(new TextBlock
                {
                    Text = orgName == "All" ? "All Organizations" : OrgLogoHelper.GetDisplayName(orgName),
                    VerticalAlignment = VerticalAlignment.Center,
                    Foreground = new Microsoft.UI.Xaml.Media.SolidColorBrush(Color.FromArgb(230, 232, 237, 243))
                });

                OrgComboBox.Items.Add(new ComboBoxItem { Content = sp, Tag = orgName });
            }
            SyncOrgComboBoxSelection();
        }
        finally
        {
            _isUpdatingOrgCombo = false;
        }
    }

    private void SyncOrgComboBoxSelection()
    {
        _isUpdatingOrgCombo = true;
        try
        {
            var target = ViewModel.SelectedOrganization ?? "All";
            foreach (ComboBoxItem item in OrgComboBox.Items)
            {
                if (item.Tag is string tag && tag == target)
                {
                    OrgComboBox.SelectedItem = item;
                    return;
                }
            }
    ...
    private void OrgComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (_isUpdatingOrgCombo) return;
        if (OrgComboBox.SelectedItem is ComboBoxItem item && item.Tag is string tag)
        {
            ViewModel.SelectedOrganization = tag;
        }
    }
}
